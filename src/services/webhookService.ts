...
export const webhookService = {
  transformFormDataToPayload(formData: FormData): WebhookPayload {
    const isCoordinator = formData.callCategory === 'coordinator';
    const isVC = formData.mediaType === 'video';
    const mediaUrl = isVC ? (formData.vcUrl || '') : (formData.audioUrl || '');
    const vcPlatform = isVC ? detectVCPlatform(mediaUrl) : undefined;

    // Sales: include selected params. Coordinator: empty (n8n uses predefined SOP)
    const customParamNames = formData.customParameters?.map(p => p.name) || [];
    const selectedParams = isCoordinator
      ? []
      : [...(formData.selectedParameters || []), ...customParamNames];

    const payload: WebhookPayload = {
      // ── Common fields ─────────────────────────────────
      call_category: formData.callCategory || 'sales',
      email: formData.email || undefined,
      call_duration: formData.callDuration || '',
      call_duration_seconds: formData.callDuration
        ? this.parseTimeToSeconds(formData.callDuration)
        : 0,
      call_type: formData.callType,
      notes: formData.notes || undefined,
      audio: {
      ...
      ...(isCoordinator && {
        coordinator_type: formData.coordinatorType || undefined,
        learner_email: formData.learnerEmail || undefined,
        lsq_id: formData.lsqId || undefined,
        lsq_link: formData.lsqLink || undefined,
        selected_parameters: [],
        custom_parameters: [],
      }),
    };
  ...
  async sendWithRetry(
    ...
    retries = WEBHOOK_CONFIG.retryAttempts
